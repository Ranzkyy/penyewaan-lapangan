...
class UserController extends Controller
{
    ...
    public function settingAkun()
    {
        ...
        // Ambil waktu pembaruan terakhir dari database
        $lastUpdated = $user->updated_at; // Ganti dengan updated_at
        $remainingTime = 0;

        if ($lastUpdated) {
            // Hitung selisih waktu dalam milidetik, 2 menit = 120000 ms
            $remainingTime = max(0, 120000 - Carbon::now()->diffInMilliseconds($lastUpdated));
        }

        return view('user.setting-akun', [
            'user' => $user,
            'remainingTime' => $remainingTime, // Waktu tersisa dalam milidetik
        ]);
    }
    ...
    public function updateAkun(Request $request)
    {
        ...
        $request->validate([
            ...
            'username' => 'required|alpha_num|min:5|max:15|unique:users,username,' . Auth::id(),
        ...
        // Simpan waktu pembaruan terakhir di database (secara otomatis dikelola oleh Eloquent)
        $user->save();
