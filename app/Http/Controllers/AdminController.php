...
class AdminController extends Controller
{
    ...
    public function booking(Request $request)
    {
        ...
        // Get semua user untuk referensi
        $user = User::when($search, function($query) use ($search) {
            return $query->where('name', 'LIKE', "%{$search}%");
        })->get();

        // Base query untuk booking
        $bookingsQuery = Booking::query();

        // Filter booking berdasarkan user yang ditemukan
        if ($search) {
            $userIds = $user->pluck('id');
            $bookingsQuery->whereIn('id_user', $userIds);
        }

        if ($request->has('daily_date')) {
            $bookingsQuery->whereDate('tgl_booking_222142', Carbon::parse($request->daily_date));
            ...
            $bookingsQuery->whereBetween('tgl_booking_222142', [$start, $end]);
            ...
            $year = $request->tahun;
            $bookingsQuery->whereMonth('tgl_booking_222142', $month)
                         ->whereYear('tgl_booking_222142', $year);
            $judul = 'Bulanan - ' . DateTime::createFromFormat('!m', $month)->format('F') . ' ' . $year;
        } elseif ($request->has('yearly_year')) {
            $year = $request->yearly_year;
            $bookingsQuery->whereYear('tgl_booking_222142', $year);
            $judul = 'Tahunan - ' . $year;
        } else {
